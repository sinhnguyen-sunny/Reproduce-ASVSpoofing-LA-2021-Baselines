...
import math
import h5py
from extract_cqcc_feature import extract_cqcc_feature
...
def extract_cqcc(sig, fs, fmin, fmax, B=12, cf=19, d=16):
	...
	Xcq = cqt(sig, B, fs, fmin, fmax, 'rasterize', 'full', 'gamma', gamma)
	absCQT = abs(Xcq['c'])
...
def extract_features(file, features, cached=False):
	def get_feats():
		if features == 'cqcc':
			...
			# sig = sig.reshape(sig.shape[0], 1)  # for one-channel signal
			# fmax = rate/2
			# fmin = fmax/2**9
			# B = 96
			# d = 16
			# cf = 19
			# return extract_cqcc(sig, rate, fmin, fmax)
			# fmin, fmax, B=12, cf=19, d=16

			#get code from
			#https://github.com/stoneMo/ASVspoof/blob/main/data_processing.py#L23
			# PARAMETERS
			CQcc, fmax, fmin = extract_cqcc_feature(sig, rate)			
			return CQcc.T
		else:
			return None
...
def scoring(scores_file, dict_file, features, eval_ndx, eval_folder, audio_ext, features_cached=True, flag_debug=False):
	...
	pd = pandas.read_csv(eval_ndx, sep=' ', header=None, names=["Values"])
	if flag_debug:
		pd = pd[:1000]

	# files = pd[1].values
	files = pd["Values"].values
